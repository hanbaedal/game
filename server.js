...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'index.html'));
        ...
        // 사용자별 배팅 데이터 초기화 API (새로운 게임에서 배팅할 수 있도록)
        app.post('/api/clear-user-betting', async (req, res) => {
            try {
                ...
                console.log(`🧹 사용자 배팅 데이터 초기화: ${userId} - 게임 ${gameNumber}`);
                ...
                if (gameData && gameData.bets) {
                    // 해당 사용자의 배팅만 제거
                    const filteredBets = gameData.bets.filter(bet => bet.userId !== userId);
                    
                    if (filteredBets.length !== gameData.bets.length) {
                        // betCounts 재계산
                        const newBetCounts = {
                            '1루': 0, '2루': 0, '3루': 0, '홈런': 0, '삼진': 0, '아웃': 0
                        };
                        ...
                        console.log(`✅ 사용자 ${userId} 배팅 데이터 초기화 완료: 게임 ${gameNumber}`);
                        ...
                        res.json({
                            ...
                            data: {
                                userId: userId,
                                gameNumber: gameNumber,
                                removedBets: gameData.bets.length - filteredBets.length
                            }
                        });
                    } else {
                        // 이미 배팅 데이터가 없는 경우
        ...
        app.post('/api/activate-first-game', async (req, res) => {
            try {
                ...
                // team-games 컬렉션에서 첫 번째 게임 찾기
                const teamGamesCollection = getTeamGamesCollection();
                const firstGame = await teamGamesCollection.findOne({ 
                    date: todayString, 
                    gameNumber: 1 
                });
                ...
                if (!firstGame) {
                    return res.status(404).json({ 
                        success: false,
                        message: '첫 번째 경기를 찾을 수 없습니다.' 
                    });
                }
                
                // 첫 번째 게임을 활성화 상태로 업데이트
                await teamGamesCollection.updateOne(
                    { _id: firstGame._id },
                    { 
                        $set: { 
                ...
                console.log(`✅ 첫 번째 게임 자동 활성화 완료: 경기 1 (${firstGame.matchup})`);
                
                res.json({ 
                    success: true,
                    message: '첫 번째 게임이 활성화되었습니다.',
                    game: {
                        gameNumber: 1,
                        matchup: firstGame.matchup,
                        bettingStart: '시작',
                        bettingStop: '진행'
                    }
                });
                ...
                console.error('첫 번째 게임 자동 활성화 오류:', error);
                res.status(500).json({
                    success: false,
